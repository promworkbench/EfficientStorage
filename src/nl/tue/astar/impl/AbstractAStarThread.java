...
public abstract class AbstractAStarThread<H extends Head, T extends Tail> implements ObservableAStarThread<H, T> {
	...
	public static interface StorageHandler<H extends Head, T extends Tail> {
		 ...
		 * @throws AStarException
		 */
		public <TT extends FastLowerBoundTail> void reComputeFastLowerboundTail(Record rec, TT tail, H head,
				int storedEstimate) throws AStarException;
	 ...
	 */
	public AbstractAStarThread(Delegate<H, T> delegate, Trace trace, int maxStates, StorageHandler<H, T> storageHandler) {
		this.delegate = delegate;
		this.trace = trace;
		this.maxStates = maxStates;
		this.storageHandler = storageHandler;
		this.queue = new DepthFirstFastLookupPriorityQueue(1000);
	...
	public void setPreferBreadth(boolean preferBreadth) {
		setQueueingModel(preferBreadth ? QueueingModel.BREADTHFIRST : QueueingModel.DEPTHFIRST);
	}
	...
	public void setQueueingModel(QueueingModel model) {
		switch (model) {
			...
			case RANDOM :
				...
				break;
			case BREADTHFIRSTWITHCERTAINTYPRIORITY :
				setQueue(new BreadthFirstFastLookupPriorityQueue(1000, true));
				break;
			case DEPTHFIRSTWITHCERTAINTYPRIORITY :
				setQueue(new DepthFirstFastLookupPriorityQueue(1000, true));
				break;
		}
	}
	...
	public Record getOptimalRecord(final Canceller c, final int stopAt, final double timeLimit) throws AStarException {
		...
		while (!queue.isEmpty() && !c.isCancelled() && (timeLimit < 0 || System.currentTimeMillis() < endTime)) {
	...
	protected void processMovesForRecord(Record rec, H head, T tail, int stopAt, double timeLimit, long endTime)
			throws AStarException {
		...
		if (!rec.isExactEstimate()) {
			...
			if (!wasExact) {
				...
				storageHandler.reComputeFastLowerboundTail(rec, ftail, head, recEstimate);
			}
			...
			if (recEstimate == tailEstimate) {
				if (type == Type.PLAIN) {
					...
					rec.setEstimatedRemainingCost(rec.getEstimatedRemainingCost(), true);
				} else {
		...
		switch (type) {
			...
			case WEIGHTED_DYNAMIC :
				if (rec.getBacktraceSize() < expectedLength) {
					rec.setEstimatedRemainingCost((1 + epsilon * (1 - rec.getBacktraceSize() / expectedLength))
							* estimate, true);
				} else {
					rec.setEstimatedRemainingCost(estimate, true);
				}
	...
	protected void processMovesForRecordWithUpToDateTail(Record rec, H head, T tail, int stopAt, double timeLimit,
		...
		while (evtIt.hasNext()) {
			...
			while (it.hasNext()) {
				int trans = it.next();
				if (allowMove(rec, trans, nextEvent)) {
					processMove(head, tail, rec, trans, nextEvent, activity);
				}
			}
			...
			if (isValidMoveOnLog(rec, nextEvent, activity, enabled, ml)) {
				...
				// 3) a log move.
				if (allowMove(rec, NOMOVE, nextEvent)) {
		...
		if (isValidMoveOnModel(rec, nextEvents, activity, enabled)) {
			...
			while (it.hasNext()) {
				// move model
				int trans = it.next();
				if (allowMove(rec, trans, NOMOVE)) {
					processMove(head, tail, rec, trans, NOMOVE, NOMOVE);
				}
			}
		}
	...
	/**
	 * This method handles the filtering of moves in the situation the total
	 * ordering is implied.
	 * 
	 * We assume a total ordering > on all moves (l,m) is the requested move
	 * (l',m') is the preceeding move
	 * 
	 * (l'm') blocks the occurrence of (l,m) if and only if: 1) (l,m) was
	 * enabled before, i.e. no token flow 2) (l'm') > (l,m)
	 * 
	 * Assume T(model) > T(log) > T(sync)
	 * 
	 * Interpretation is: You can only do model or log moves with the intent to
	 * enable sync moves and among them, model and log moves are scheduled log
	 * move first
	 * 
	 * 
	 * @param rec
	...
	protected boolean allowMove(Record rec, int modelMove, int logMove) throws AStarException {
		if (sorting != ASynchronousMoveSorting.TOTAL || rec.getPredecessor() == null) {
			// no total ordering, or initial state
			return true;
		}
		int modelMovePrime = rec.getModelMove();
		int logMovePrime = rec.getMovedEvent();

		// log move only if previously a log move or a sync move.
		if (modelMove == NOMOVE) {
			if (logMovePrime != NOMOVE) {
				return true;
			} else {
				return false;
			}
		} else if (modelMovePrime == NOMOVE) {
			// model move after a logmove.
			return true;
		}

		if (logMovePrime != NOMOVE && logMove != NOMOVE) {
			// two sync moves, allow since there is a dependency in the events
			return true;
		}

		// check if modelMove was enabled before
		TIntList enabled = storageHandler.getStoredState(rec.getPredecessor()).getHead()
				.getModelMoves(rec.getPredecessor(), delegate);
		if (enabled.contains(modelMove)) {
			// modelMove was possible before, so no dependency in the model. 
			if (logMovePrime == NOMOVE && logMove == NOMOVE) {
				// two parallel model moves
				// allow only in specific order
				if (modelMovePrime <= modelMove) {
					return true;
				} else {
					return false;
				}
			} else {
				// one of the two is sync move, that goes first
				// so only allow if modelMove is NOMOVE.
				if (modelMove == NOMOVE) {
					return true;
				} else {
					return false;
	...
	protected void processMove(H head, T tail, Record rec, int modelMove, int movedEvent, int activity)
		...
		// to compute the tail.
		newTail = computeNewTail(newRec, tail, newHead, modelMove, movedEvent, activity);
	...
	protected T computeNewTail(Record newRec, T tail, H newHead, int modelMove, int movedEvent, int activity) {
		...
		T newTail = (T) tail.getNextTail(delegate, newHead, modelMove, movedEvent, activity);
		...
		if (newTail instanceof FastLowerBoundTail) {
			...
			if (!ftail.isExactEstimateKnown()) {
				...
				if (queue.isEmpty() || queue.peek().getTotalCost() >= newRec.getCostSoFar() + est) {
		...
		setExactEstimateForRecord(newRec, newTail.getEstimatedCosts(delegate, newHead));
