...
public class MemoryEfficientStorageHandler<H extends Head, T extends Tail> implements
	...
	public int getEstimate(H head, long index) throws AStarException {
		...
		try {
			synchronized (stream.getLock()) {
				return delegate.getTailInflater().inflateEstimate(delegate, head, stream);
			}
		} catch (IOException e) {
			throw new AStarException(e);
		}
	...
	public <TT extends FastLowerBoundTail> void reComputeFastLowerboundTail(Record rec, TT tail, H head,
		...
		if (!tail.isExactEstimateKnown()) {
			...
			// synchronized (statespace) {
			SkippableOutputStream out = store.getOutputStreamForObject(rec.getState());
			// rewrite the tail in this outputstream
			try {
				((CachedStorageAwareDelegate<H, TT>) delegate).getHeadDeflater().skip(head, out);
				synchronized (out.getLock()) {
					((CachedStorageAwareDelegate<H, TT>) delegate).getTailDeflater().deflate(tail, out);
				}
			} catch (IOException e) {
				throw new AStarException(e);
			}
